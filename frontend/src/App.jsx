...
import { LogOut, Search, MessageCircle, Banknote, UserMinus, MousePointer2, Calendar, ShieldCheck, Lock, RefreshCw, Loader2, Send, Trash2, Camera, Plus, Minus, Home, LayoutGrid, List, Edit2 } from 'lucide-react';
  ...
  return (
    <div className="flex items-center justify-center h-screen bg-gradient-to-br from-slate-100 to-blue-100 font-sans">
      <div className="bg-white p-10 rounded-2xl shadow-2xl w-96 border border-white/50 backdrop-blur-sm">
        ...
        <h2 className="text-3xl font-bold mb-2 text-slate-800 text-center">Welcome</h2>
        {msg.text && <div className={`p-3 rounded-lg mb-6 text-sm font-medium text-center ${msg.type === 'success' ? 'bg-green-50 text-green-600 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>{msg.text}</div>}
        <form onSubmit={handleSubmit} className="space-y-5">
            <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1 ml-1">Username</label><input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" onChange={e => setCreds({...creds, username: e.target.value})} /></div>
            <div><label className="block text-xs font-bold text-slate-500 uppercase mb-1 ml-1">Password</label><input className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500" type="password" onChange={e => setCreds({...creds, password: e.target.value})} /></div>
            <button disabled={loading} className="w-full bg-blue-600 text-white p-3.5 rounded-xl font-bold shadow-lg hover:bg-blue-700 flex justify-center gap-2 mt-4">{loading ? <Loader2 className="animate-spin"/> : "Sign In"}</button>
        </form>
      </div>
    </div>
  );
...
function AdminPanel({ user, onLogout }) {
    ...
    const toggleStatus = async (userId, currentStatus) => { await axios.post(`${API}/admin/approve`, { userId, status: currentStatus === 1 ? 0 : 1 }); axios.get(`${API}/admin/users`).then(res => setOwners(res.data)); };
    const handleDeleteOwner = async (userId) => { if(confirm(`Delete Owner?`)) { await axios.post(`${API}/admin/delete-owner`, { userId }); axios.get(`${API}/admin/users`).then(res => setOwners(res.data)); } };
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans">
            <nav className="bg-slate-900 text-white p-4 shadow-lg sticky top-0 z-20 flex justify-between"><h1 className="font-bold flex gap-2"><ShieldCheck/> Admin</h1> <button onClick={onLogout} className="bg-red-600 px-4 py-1 rounded">Logout</button></nav>
            <div className="p-8 max-w-4xl mx-auto space-y-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm flex gap-4 items-end">
                    <div className="flex-1"><label className="text-xs font-bold">New Admin Password</label><input className="w-full border p-2 rounded" value={newPass} onChange={e => setNewPass(e.target.value)} /></div>
                ...
                <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
                    <table className="w-full text-sm text-left"><thead className="bg-slate-100 text-slate-500 uppercase"><tr><th className="p-4">User</th><th className="p-4">Hostel</th><th className="p-4">Status</th><th className="p-4">Action</th></tr></thead>
                        <tbody>{owners.map(o => (<tr key={o.id} className="border-b hover:bg-slate-50"><td className="p-4 font-bold">{o.username}</td><td className="p-4">{o.hostel_name}</td><td className="p-4">{o.is_approved===1?'Active':'Pending'}</td><td className="p-4 flex gap-2"><button onClick={()=>toggleStatus(o.id, o.is_approved)} className="bg-blue-600 text-white px-3 py-1 rounded">{o.is_approved===1?'Block':'Approve'}</button><button onClick={()=>handleDeleteOwner(o.id)} className="bg-red-100 text-red-600 p-2 rounded"><Trash2 size={16}/></button></td></tr>))}</tbody>
                    </table>
                </div>
            </div>
        </div>
...
function SetupPage({ user, onUpdate }) {
  ...
  // 👇 FIXED LOGIC: Toggle instead of Resetting 👇
  const handleMouseDown = (i, e) => { 
      e.preventDefault(); 
      setIsDragging(true); 
      
      const newSet = new Set(selectedIndices);
      if(newSet.has(i)) newSet.delete(i); // Unselect if tapped again
      else newSet.add(i); // Add if new tap
  ...
  const getColor = (c) => { if(c===1) return 'bg-slate-100 border-slate-200 text-slate-500'; if(c===2) return 'bg-blue-50 border-blue-200 text-blue-600'; if(c===3) return 'bg-purple-50 border-purple-200 text-purple-600'; return 'bg-orange-50 border-orange-200 text-orange-600'; };

  return (
    <div className="min-h-screen bg-slate-50 flex flex-col p-10 items-center">
        {step === 1 ? (
          <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md space-y-4">
            <h2 className="text-2xl font-bold">Setup Hostel</h2>
            <input className="w-full border p-3 rounded" placeholder="Hostel Name" onChange={e => setConfig({...config, hostelName: e.target.value})} />
            <select className="w-full border p-3 rounded" onChange={e => setConfig({...config, maxFloor: parseInt(e.target.value)})}>{[0,1,2,3,4,5,6,7,8].map(n => <option value={n}>Ground + {n} Floors</option>)}</select>
            <div className="flex gap-2"><input className="w-1/2 border p-3 rounded" placeholder="Start (01)" onChange={e => setRange({...range, start: e.target.value})} /><input className="w-1/2 border p-3 rounded" placeholder="End (10)" onChange={e => setRange({...range, end: e.target.value})} /></div>
            <button onClick={handleGenerate} className="w-full bg-blue-600 text-white p-3 rounded font-bold">Next</button>
          </div>
        ) : (
          <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-4xl h-[80vh] flex flex-col">
            <h2 className="text-xl font-bold mb-4">Tap to Select Multiple Rooms</h2>
            <div className="flex-1 overflow-y-auto grid grid-cols-5 gap-2 p-2 border rounded-xl mb-4 bg-slate-50 select-none" onMouseUp={() => setIsDragging(false)}>
                {generatedRooms.map((r, i) => (
                    <div key={i} 
                        ...
                        className={`p-2 border rounded-lg text-center cursor-pointer flex flex-col justify-center h-16 transition-all 
                        ${selectedIndices.has(i) ? 'bg-blue-600 text-white border-blue-800 scale-95' : getColor(r.capacity)}`}
                    >
                        <div className="font-bold">{r.roomNo}</div>
                        <div className="text-xs">{r.capacity} Beds</div>
                    </div>
                ))}
            </div>
            <div className="flex gap-2 justify-center pb-2">
                {[1,2,3,4,5].map(n => (
                    <button key={n} onClick={()=>applyCap(n)} className="flex-1 bg-slate-100 py-3 rounded-lg font-bold hover:bg-slate-200 border text-sm">
                        {n} Sharing
                    </button>
                ))}
            </div>
            <button onClick={submitSetup} className="w-full bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg">{loading?<Loader2 className="animate-spin mx-auto"/>:"Save & Finish"}</button>
          </div>
        )}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
      <div className="sticky top-0 z-20 bg-white/90 backdrop-blur-md border-b border-slate-200 px-4 py-3 shadow-sm flex justify-between items-center">
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2"><Home size={20}/> {user.hostel_name}</h1>
      ...
      <div className="max-w-7xl mx-auto w-full px-4 py-6">
          <div className="flex justify-center mb-6">
              <div className="bg-white p-1 rounded-xl shadow-sm border border-slate-200 inline-flex">
                  <button onClick={() => setActiveTab('overview')} className={`px-6 py-2 rounded-lg text-sm font-bold ${activeTab === 'overview' ? 'bg-blue-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>Overview</button>
                  <button onClick={() => setActiveTab('rent')} className={`px-6 py-2 rounded-lg text-sm font-bold ${activeTab === 'rent' ? 'bg-green-600 text-white shadow-md' : 'text-slate-500 hover:bg-slate-50'}`}>Rent Collection</button>
              </div>
          </div>
...
// --- IMPORT MODAL & BOOKING MODAL & RENT MODAL (Kept same) ---
...
function RentModal({ data, close }) {
  ...
  return (<div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50"><div className="bg-white rounded-3xl shadow-xl w-full max-w-sm text-center overflow-hidden"><div className="bg-green-500 p-6 text-white"><h2 className="text-2xl font-bold mb-1">{bed.clientName}</h2><p className="opacity-90 text-sm">Room {data.room.number}</p></div><div className="p-6 space-y-4"><div><p className="text-xs font-bold text-slate-400 uppercase">Rent Due Date</p><p className="text-lg font-bold text-slate-700">{dueDate}</p></div><a href={`https://wa.me/91${bed.clientMobile}?text=Rent%20Due!`} target="_blank" className="flex items-center justify-center gap-2 w-full border py-3 rounded-xl font-bold text-slate-600 hover:bg-slate-50 transition-colors"><MessageCircle size={18}/> Reminder</a><button onClick={handleMarkPaid} className="flex items-center justify-center gap-2 w-full bg-green-600 text-white py-3 rounded-xl font-bold shadow-lg hover:bg-green-700"><Banknote size={18}/> Mark Paid</button><button onClick={close} className="text-sm text-slate-400 hover:text-slate-600">Close</button></div></div></div>);
}
