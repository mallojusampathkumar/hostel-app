...
// --- FINAL EMAIL FIX ---
// We force the connection to avoid timeouts on Render
const transporter = nodemailer.createTransport({
    ...
    ignoreTLS: false,
    requireTLS: true,
    logger: true, // Log everything so we can see what's happening
    debug: true
});
    ...
    try {
        ...
        if (!user) {
            ...
            if(email && process.env.EMAIL_USER) {
                console.log("Attempting Welcome Email...");
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        ...
        // 4. Send Email
        console.log("Sending email via SMTP/465...");
        ...
        res.status(500).json({ error: "Email server timeout. Please try again." }); 
    }
});

// --- REST OF ROUTES (UNCHANGED) ---
// (Paste the standard routes for admin, setup, dashboard, etc. from previous steps here to keep the file complete)
...
app.get('/api/finance/:userId', async (req, res) => { const { userId } = req.params; const month = new Date().toISOString().slice(0, 7); try { const potentialRes = await query(`SELECT SUM(rent_amount) as total FROM beds JOIN rooms ON beds.room_id=rooms.id WHERE rooms.user_id=$1 AND is_occupied=1`, [userId]); const collectedRes = await query(`SELECT SUM(amount) as collected FROM rent_history JOIN beds ON rent_history.bed_id=beds.id JOIN rooms ON beds.room_id=rooms.id WHERE rooms.user_id=$1 AND month=$2`, [userId, month]); const expRes = await query(`SELECT * FROM expenses WHERE user_id=$1 ORDER BY date DESC`, [userId]); const workRes = await query(`SELECT * FROM workers WHERE user_id=$1`, [userId]); const totalRent = potentialRes.rows[0].total || 0; const collectedRent = collectedRes.rows[0].collected || 0; const totalExpenses = expRes.rows.reduce((sum, e) => sum + e.amount, 0); const totalSalaries = workRes.rows.reduce((sum, w) => sum + w.salary, 0); res.json({ rent: { total: totalRent, collected: collectedRent, pending: totalRent - collectedRent }, expenses: expRes.rows, workers: workRes.rows, summary: { income: collectedRent, outflow: totalExpenses + totalSalaries, profit: collectedRent - (totalExpenses + totalSalaries) } }); } catch(e) { res.status(500).json({error:e.message}); } });
app.post('/api/expenses/add', async (req, res) => { const { userId, title, amount, category } = req.body; try { await query('INSERT INTO expenses (user_id, title, amount, date, category) VALUES ($1,$2,$3,$4,$5)', [userId, title, amount, new Date().toISOString().slice(0,10), category]); res.json({success:true}); } catch(e){res.status(500).json(e);} });
app.post('/api/expenses/delete', async (req, res) => { await query('DELETE FROM expenses WHERE id=$1', [req.body.id]); res.json({success:true}); });
app.post('/api/workers/add', async (req, res) => { const { userId, name, role, salary } = req.body; try { await query('INSERT INTO workers (user_id, name, role, salary) VALUES ($1,$2,$3,$4)', [userId, name, role, salary]); res.json({success:true}); } catch(e){res.status(500).json(e);} });
app.post('/api/workers/delete', async (req, res) => { await query('DELETE FROM workers WHERE id=$1', [req.body.id]); res.json({success:true}); });
app.get('/api/rent-history/:bedId', async (req, res) => { try { const r = await query('SELECT * FROM rent_history WHERE bed_id=$1 ORDER BY month DESC LIMIT 3', [req.params.bedId]); res.json(r.rows); } catch(e){ res.status(500).json(e); } });
...
app.post('/api/pay-rent', async (req, res) => { const { bedId, monthString, amount } = req.body; try { await query('UPDATE beds SET last_rent_paid=$1 WHERE id=$2', [monthString, bedId]); await query('INSERT INTO rent_history (bed_id, amount, month, paid_date) VALUES ($1,$2,$3,$4)', [bedId, amount, monthString, new Date().toISOString().slice(0,10)]); res.json({success:true}); } catch (e) { res.status(500).json(e); } });
