...
// You must add EMAIL_USER and EMAIL_PASS to your Render Environment Variables later
...
// This runs once to add new columns (email, rent_amount) if they are missing
...
// 1. LOGIN / REGISTER (Updated with Email & Auto-Approve)
...
app.post('/api/forgot-password', async (req, res) => {
    ...
    try {
        ...
        await transporter.sendMail({
            from: process.env.EMAIL_USER,
            to: email,
            subject: 'Password Reset - Hostel Manager',
...
// --- EXISTING ROUTES (Unchanged Logic, added Rent Amount support) ---

app.post('/api/book', async (req, res) => {
    const { bedId, clientName, clientMobile, joinDate, leaveDate, advance, maintenance, rentAmount } = req.body;
    try { 
        await query(
            `UPDATE beds SET is_occupied = 1, client_name = $1, client_mobile = $2, join_date = $3, leave_date = $4, advance_amount = $5, maintenance_charges = $6, rent_amount = $7, last_rent_paid = NULL WHERE id = $8`, 
            [clientName, clientMobile, joinDate, leaveDate, advance, maintenance, rentAmount, bedId]
        ); 
...
app.post('/api/update-tenant', async (req, res) => {
    const { bedId, advance, maintenance, rentAmount, leaveDate } = req.body;
    try { 
        await query(
            `UPDATE beds SET advance_amount = $1, maintenance_charges = $2, rent_amount = $3, leave_date = $4 WHERE id = $5`, 
            [advance, maintenance, rentAmount, leaveDate || null, bedId]
        ); 
        res.json({ success: true }); 
    } catch (err) { res.status(500).json({ error: err.message }); }
});

// --- REST OF ROUTES (COPY PASTE SAME AS BEFORE) ---
